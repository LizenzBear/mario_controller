...
class MarioController:
    ...
    def calibrate(self, landmark_list):
        if np.sum(self.movement_history) < self.calibration_movement_threshold:
            ...
            hand_left_y = landmark_list[15].y
            hand_right_y = landmark_list[16].y

            # Set stop_threshold just above the average hand y position
            average_hand_y = (hand_left_y + hand_right_y) / 2
            self.stop_threshold = average_hand_y + 0.15  # Adjust delta as needed

            # Set jump_threshold just above the nose y position
            self.jump_threshold = nose_y - 0.05  # Adjust delta as needed

            # Ensure thresholds are within [0,1] range
        ...
        while True:
            ...
            if results.pose_landmarks:
                ...
                if self.in_frame(results.pose_landmarks.landmark):
                    ...
                    self.calibrate(results.pose_landmarks.landmark, frame_height)
